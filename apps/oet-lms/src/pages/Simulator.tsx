...
export default function Simulator() {
  ...
  // When backend is not available, offer demo mode — do not silently pretend live AI is on.
  useEffect(() => {
    if (connectionStatus === 'connected') setDemoMode(false)
  }, [connectionStatus])

  const patientReplyMode: 'live' | 'demo' | 'waiting' =
  ...
  const handleSend = () => {
    ...
    const nextMessages = [...messages, { who: 'you' as const, text, startedAt, sentAt }]
    ...
    if (connectionStatus === 'connected') {
      sendMessage(text, startedAt, sentAt, nextMessages)
    } else if (demoMode) {
  ...
  const handleEndSession = () => {
    ...
    const record = {
      ...
      demoMode: patientReplyMode === 'demo',
    }
  ...
  return (
    <>
      ...
      {patientReplyMode === 'live' && (
        ...
        >
          Mode: LIVE LLM persona — patient replies are generated by the chat backend for the selected persona
          (not canned).
        </div>
      )}
      {patientReplyMode === 'demo' && (
        <div
          className="siya-chat-status"
          role="status"
          style={{
        ...
        >
          <span>
            <strong>Mode: DEMO (canned replies)</strong> — patient messages are a fixed rotation and are{' '}
            <em>not</em> persona-aware. Scores still run on your typing, but this is not a live AI patient.
            For live mode: run <code style={{ background: 'rgba(0,0,0,0.06)', padding: '2px 6px', borderRadius: 4 }}>npm run dev:oet-lms</code> with{' '}
            <code style={{ background: 'rgba(0,0,0,0.06)', padding: '2px 6px', borderRadius: 4 }}>PERPLEXITY_API_KEY</code> (or OpenAI) in{' '}
            <code style={{ background: 'rgba(0,0,0,0.06)', padding: '2px 6px', borderRadius: 4 }}>integrations/oet-lms-chat/.env</code>.
          </span>
      ...
      {(connectionStatus === 'connecting' ||
        ...
        (connectionStatus === 'error' && !demoMode)) && (
        <div
        ...
        >
          <span>
            {connectionStatus === 'connecting' && 'Connecting to live chat backend…'}
            {connectionStatus === 'disconnected' &&
              'Live chat backend not connected yet. You can wait, or use DEMO canned replies.'}
      ...
      <div className="siya-simulator-container">
        <div className="siya-simulator-panel">
          <div className="siya-panel-header">
            <div className="siya-panel-title">
              Chat with {patientReplyMode === 'live' ? 'LIVE LLM' : patientReplyMode === 'demo' ? 'DEMO (canned)' : 'AI'} patient
            </div>
            <div className="siya-panel-subtitle">
              {persona ? `${persona.name}, ${persona.archetype}` : 'Select a persona to start'}
              {patientReplyMode === 'demo' ? ' · DEMO replies' : patientReplyMode === 'live' ? ' · LIVE LLM' : ''}
            </div>
          </div>
        ...
        <div className="siya-chat-feedback" style={{ marginTop: 24 }}>
          ...
          {chatEvaluation ? (
            <div className="siya-session-summary">
              ...
              {patientReplyMode === 'demo' && (
                <p className="siya-chat-small" style={{ color: '#8a5a00' }}>
                  This session used <strong>DEMO canned patient replies</strong> (not live LLM).
                </p>
              )}
              {patientReplyMode === 'live' && (
                <p className="siya-chat-small" style={{ color: 'var(--siya-primary)' }}>
                  This session used <strong>LIVE LLM</strong> persona replies.
                </p>
              )}
            </div>
          ) : (
