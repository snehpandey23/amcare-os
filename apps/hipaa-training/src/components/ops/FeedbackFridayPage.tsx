...
export function FeedbackFridayPage() {
  ...
  const tourMode = isTourUrlParam(searchParams.get("tour"));
  ...
  const [tourDemoSent, setTourDemoSent] = useState<RecipientFacingFeedback | null>(null);
  ...
  const reload = useCallback(async () => {
    if (!authReady || !user || tourMode) return;
    setLoading(true);
    setError(null);
    try {
  ...
  }, [authReady, user, recipientUserId, targetKind, tourMode]);

  useEffect(() => {
    if (tourMode && !body) {
      setBody(tourFeedbackPracticeBody());
    }
  }, [tourMode, body]);
  ...
  }, [targetKind, peers, leads, recipientUserId, tourMode]);

  async function onSubmitTour(e: FormEvent) {
    e.preventDefault();
    setPending(true);
    setError(null);
    setNotice(null);
    try {
      ...
      setTourDemoSent(recipientFacing);
      setNotice(
        anonymous
          ? `Sandbox sent to ${TOUR_DEMO_PEER.label} — anonymous demo only (nothing saved or emailed).`
          : `Sandbox sent to ${TOUR_DEMO_PEER.label} as a demo (nothing saved or emailed).`,
      );
      recordTourFeedbackDone();
    } catch (err) {
      ...
      <div className={portalPage}>
        <header className="mb-6">
          <p className="text-xs text-[var(--siya-text-muted)]">
            ...
            {" · "}
            Product tour · Feedback sandbox
          </p>
          <h1 className={`${portalH1} mt-2`}>Feedback — tour sandbox</h1>
          <p className="mt-2 max-w-xl text-sm text-[var(--siya-text-secondary)]">
            Try the feedback flow with a <strong>demo colleague</strong> only. Nothing is written to the real inbox
            and no email is sent.
          </p>
        </header>

        <div className="grid gap-6 lg:grid-cols-2">
          <section className={portalSection} aria-label="Give sandbox feedback">
            <h2 className={portalH3}>Give sandbox feedback</h2>
            <form className="mt-3 space-y-3" onSubmit={(e) => void onSubmitTour(e)}>
              <label className="block text-xs font-semibold text-[var(--siya-primary)]">
                Recipient
                <input
                  className={`mt-1 ${portalInput} bg-[var(--siya-bg-subtle)]`}
                  value={TOUR_DEMO_PEER.label}
                  readOnly
                  aria-readonly="true"
                />
              </label>
              ...
              <fieldset className="space-y-2 rounded-lg border border-[var(--siya-border)] p-3">
                ...
                <label className="flex items-start gap-2 text-sm text-[var(--siya-text-secondary)]">
                  ...
                  />
                  <span>
                    <strong className="text-[var(--siya-primary)]">Share my name</strong> — demo only.
                  </span>
                </label>
                <label className="flex items-start gap-2 text-sm text-[var(--siya-text-secondary)]">
                  <input
                  ...
                  />
                  <span>
                    <strong className="text-[var(--siya-primary)]">Send anonymously</strong> — demo only.
                  </span>
                </label>
              </fieldset>
              ...
              <button type="submit" className={portalBtnAccent} disabled={pending}>
                {pending ? "Sending…" : "Send sandbox feedback"}
              </button>
            </form>
          </section>

          <section className={portalSection} aria-label="Demo preview">
            <h2 className={portalH3}>What they would see (demo)</h2>
            <p className="mt-1 text-[11px] text-[var(--siya-text-muted)]">
              Preview only — not stored in anyone&apos;s real inbox.
            </p>
            {tourDemoSent ? (
              <div
                className="mt-3 rounded-lg border border-[var(--siya-border)] bg-[var(--siya-bg-page)] px-3 py-2"
                data-feedback-id={tourDemoSent.id}
                data-tour-sandbox="1"
              >
                <p className="text-[11px] font-semibold text-[var(--siya-primary)]">
                  {tourDemoSent.attribution.mode === "anonymous"
                    ? "Anonymous teammate"
                    : tourDemoSent.attribution.displayName}
                  {" · "}
                  peer note (demo)
                </p>
                <p className="mt-1 text-sm text-[var(--siya-text-secondary)]">{tourDemoSent.body}</p>
                <p className="mt-1 text-[10px] text-[var(--siya-text-muted)]">Just now · sandbox</p>
              </div>
            ) : (
              <p className="mt-3 text-sm text-[var(--siya-text-muted)]">
                Send the sandbox note to preview how feedback appears in-app.
              </p>
            )}
          </section>
        </div>
    ...
    <div className={portalPage}>
      <header className="mb-6">
        ...
        <p className="mt-2 max-w-xl text-sm text-[var(--siya-text-secondary)]">
          ...
          anonymously. Anonymous notes show <strong>no</strong> name, email, or team. Feedback works{" "}
