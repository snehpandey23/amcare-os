...
const BRIBE_MONEY_PATTERNS = [
  ...
  /\b(pay|venmo|zelle|cash\s*app|cashapp|paypal|tip|apple\s*pay)\s+me\b/i,
  /\byou\s+can\s+(pay|venmo|zelle|tip|send)\s+me\b/i,
  /\bsend\s+me\s+(some\s+)?(money|cash|usdt|btc|bitcoin|crypto|eth)\b/i,
  ...
  /\bsend\s+(the\s+)?payment\s+to\s+my\s+(personal\s+)?account\b/i,
  /\b(drop|put)\s+(some\s+)?(cash|money)\s+on\s+my\s+(cash\s*app|cashapp|venmo|zelle)\b/i,
  /\bmy\s+(cash\s*app|cashapp|venmo|zelle)\b.{0,48}\b(tip|pay|send|rush|bump)\b/i,
  /\btake\s+care\s+of\s+me\s+financially\b/i,
  /\b(financially|extra\s+cash|side\s+payment).{0,48}\b(priority|bump|rush|front\s+of\s+the\s+line)\b/i,
  /\bgive\s+me\s+(some\s+)?(cash|money)\b/i,
  /\b(pay|money|cash|tip|funds)\b.{0,48}\b(via|through|on|with)\s+(venmo|zelle|cash\s*app|cashapp|paypal|apple\s*pay)\b/i,
  /\b(via|through|on|with)\s+(venmo|zelle|cash\s*app|cashapp|paypal|apple\s*pay)\b.{0,48}\b(pay|money|cash|tip|funds)\b/i,
...
const PERSONAL_PAYMENT_SOLICIT_MARKERS = [
  ...
  /\bsend\s+(the\s+)?payment\s+to\s+my\s+(personal\s+)?account\b/i,
  /\b(drop|put)\s+(some\s+)?(cash|money)\s+on\s+my\s+(cash\s*app|cashapp|venmo|zelle)\b/i,
  /\bmy\s+(cash\s*app|cashapp|venmo|zelle)\b/i,
  /\btake\s+care\s+of\s+me\s+financially\b/i,
  /\bgive\s+me\s+(some\s+)?(cash|money)\b/i,
  /\bunder\s+the\s+table\b/i,
  /\bbribe\b/i,
  /\b(via|through|on|with)\s+(venmo|zelle|cash\s*app|cashapp|paypal|apple\s*pay)\b/i,
];

const CLINICAL_DECISION_PATTERNS = [
  /i'?ll\s+(get|give|send|write)\s+you\s+(the\s+)?(adderall|prescription|rx|meds|medication|script)\b/i,
  /i\s+can\s+(get|give|send|write)\s+you\s+(the\s+)?(adderall|prescription|rx|meds|medication|script)\b/i,
  /i\s+can\s+write\s+you\s+a\s+script\b/i,
  ...
  /you\s+don'?t\s+need\s+(the\s+)?(neurocognitive|creyos|assessment|evaluation|agreement)/i,
  /i'?ll\s+get\s+your\s+meds?\s+going/i,
...
const SCREENING_AS_DIAGNOSIS_PATTERNS = [
  ...
  /\bscreenings?\b.{0,40}\bconfirms?\s+(your|the|a)\s+\w+\s+diagnos/i,
  /\bscreenings?\b.{0,60}\b(you'?ll|you\s+will)\s+know\s+(whether|if)\s+you\s+have\b/i,
  /\bafter\s+(the\s+)?screenings?\b.{0,40}\b(know|tell).{0,24}\b(whether|if)\s+you\s+have\b/i,
